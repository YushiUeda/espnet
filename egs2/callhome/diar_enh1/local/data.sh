...
# Copyright 2022 Carnegie Mellon University (Yushi Ueda)
...
callhome_dir=/ocean/projects/cis210027p/shared/corpora/callhome_diar/LDC2001S97
swb2_phase1_train=/ocean/projects/cis210027p/shared/corpora/callhome_diar/LDC98S75
data_root=/ocean/projects/cis210027p/shared/corpora/callhome_diar
musan_root=/ocean/projects/cis210027p/shared/corpora/callhome_diar/musan
...
simu_opts_num_speaker_array=2 # (1 2 3 4) for 4 speaker setting  ("2" for 2 speaker setting)
simu_opts_sil_scale_array=2 # (2 2 5 9) for 4 speaker setting  ("2" for 2 speaker setting)
simu_opts_rvb_prob=0.5
#simu_opts_num_train=100000
simu_opts_num_train=10000
...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
    ...
    if ! utils/validate_data_dir.sh --no-text --no-feats data/callhome1_spkall \
        || ! utils/validate_data_dir.sh --no-text --no-feats data/callhome2_spkall; then
    ...
    if ! utils/validate_data_dir.sh --no-text --no-feats data/musan_noise_bg; then
    ...
    # simu rirs 8k
    if ! utils/validate_data_dir.sh --no-text --no-feats data/simu_rirs_8k; then
    ...
    if ! utils/validate_data_dir.sh --no-text --no-feats data/swb_sre_cv; then
        utils/copy_data_dir.sh data/swb_sre_comb data/swb_sre_comb_seg
        ...
        utils/fix_data_dir.sh data/swb_sre_comb_seg
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    ...
    random_mixture_cmd=local/simulation/random_mixture_nooverlap.py
    make_mixture_cmd=local/simulation/make_mixture_nooverlap.py
    if [ "$simu_opts_overlap" == "yes" ]; then
        random_mixture_cmd=local/simulation/random_mixture.py
        make_mixture_cmd=local/make_mixture_2.py
    fi
        ...
        for dset in swb_sre_tr swb_sre_cv; do
            ...
            if ! utils/validate_data_dir.sh --no-text --no-feats $simudir/data/$simuid; then
                ...
                $train_cmd JOB=1:$nj $simudir/.work/make_mixture_$simuid.JOB.log \
                    $make_mixture_cmd --rate=8000 --num_spk=${simu_opts_num_speaker}\
                    $simudir/.work/mixture_$simuid.JOB.scp \
                    $simudir/.work/data_$simuid.JOB $simudir/wav/$simuid/JOB
                extra_files="noise1.scp"
                for i in $(seq ${simu_opts_num_speaker}); do
                    extra_files+=" spk$i.scp"
                done
                utils/combine_data.sh --extra_files "${extra_files}" $simudir/data/$simuid $simudir/.work/data_$simuid.*
                steps/segmentation/convert_utt2spk_and_segments_to_rttm.py \
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
    ...
    if ! utils/validate_data_dir.sh --no-text --no-feats $adapt_set; then
