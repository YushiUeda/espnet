...
simu_opts_num_speaker_array=(1 2 3 4) # for 4 speaker setting  ("2" for 2 speaker setting)
simu_opts_sil_scale_array=(2 2 5 9) # for 4 speaker setting  ("2" for 2 speaker setting)
...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
    ...
    if ! utils/validate_data_dir.sh --no-text --no-feats data/callhome1_spkall \
        ...
        for dset in callhome1 callhome2; do
            ...
            utils/utt2spk_to_spk2utt.pl data/${dset}_spkall/utt2spk > data/${dset}_spkall/spk2utt
            utils/fix_data_dir.sh data/${dset}_spkall
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    ...
    if [ "$simu_opts_overlap" == "yes" ]; then
        ...
        make_mixture_cmd=local/simulation/make_mixture.py
    fi
...
for dset in swb_sre_tr swb_sre_cv; do
    ...
    simuid_concat=${dset}_ns"$(IFS="n"; echo "${simu_opts_num_speaker_array[*]}")"_beta"$(IFS="n"; echo "${simu_opts_sil_scale_array[*]}")"_${n_mixtures}
    utils/fix_data_dir.sh $simudir/data/$simuid_concat
done
